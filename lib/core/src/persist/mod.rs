...
impl Persister {
    ...
    pub(crate) fn list_ongoing_swaps(&self) -> Result<Vec<Swap>> {
        ...
        let ongoing_chain_swaps: Vec<Swap> = self
            ...
            .map(Swap::Chain)
            .collect();
        Ok([ongoing_send_swaps, ongoing_receive_swaps, ongoing_chain_swaps].concat())
    }
    ...
    fn sql_row_to_payment(&self, row: &Row) -> Result<Payment, rusqlite::Error> {
        ...
        let maybe_chain_swap_payer_amount_sat: Option<u64> = row.get(21)?;
        let maybe_chain_swap_receiver_amount_sat: Option<u64> = row.get(22)?;
        let maybe_chain_swap_refund_tx_id: Option<String> = row.get(23)?;
